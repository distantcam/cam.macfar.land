function postLink(liquidEngine) {
  return {
    ...
    render: function (scope, hash) {
      ...
      let path = isQuoted
        ? liquidEngine.evalValue(this.args, scope)
        : this.args;

      let results = scope.environments.collections.posts.filter(function (
        tmpl
      ) {
      ...
      return Promise.resolve(
        `<a href="${results[0].url}" title="${results[0].data.title}">${results[0].data.title}</a>`
      );
    },
  };
}
